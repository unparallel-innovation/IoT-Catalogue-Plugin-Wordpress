...
class IoTCat_data_concepts  extends IoTCat_elements{

	function __construct($name = "Data Concepts", $singular_name = "Data Concept",$default_metadata,$comment_type, $post_type = "iotcat_data_concept") {
		parent::__construct($name, $singular_name, $post_type,$default_metadata,$comment_type );

		add_action('init',array($this,'create_taxonomies'));

		add_action('manage_iotcat_data_concept_posts_columns',array($this,'columns'),10,2);
		add_action('manage_iotcat_data_concept_posts_custom_column',array($this,'column_data'),11,2);


	}

	public static $page_name = "dataConcepts";
	...
	function create_taxonomies() {
		...
		register_taxonomy('iotcat_data_concept_type',array($this->post_type),$args);
		...
		register_taxonomy('iotcat_data_concept_attribute',$this->post_type,$args);
	}

	function columns($columns) {
		unset($columns['date']);
		unset($columns['taxonomy-iotcat_data_concept_attribute']);
		unset($columns['comments']);
		unset($columns['author']);
		return array_merge(
			$columns,
			array(
				'iotcat_data_concept_original_id' => 'Original Id',
	...
	function column_data($column,$post_id) {
		switch($column) {
			case 'iotcat_data_concept_original_id' :
